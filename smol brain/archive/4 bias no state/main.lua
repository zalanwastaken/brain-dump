    ...
    return({
        _guified = {
            ...
            draw = function()
                ...
                love.graphics.setColor(colors.green)
                love.graphics.print("Success rate"..tostring((good/total)*100).."%", 0, 0)

                love.graphics.setColor(colors.red)
                love.graphics.print("Error rate"..tostring((bad/total)*100).."%", 0, 12)
            end,
            ...
            update = function()
                ...
                if newDist>oldDist then --? was this a good move?
                    ...
                    bias[movesel] = bias[movesel] - 2
                    bias[movesel] = math.max(-20, math.min(20, bias[movesel]))
                else
                    dots[#dots].good = true
                    bias[movesel] = bias[movesel] + 1
                end
    ...
    _guified = {
        ...
        update = function()
            ...
            if x > love.graphics.getWidth() or x < 0 or y > love.graphics.getHeight() or y < 0 then
                ...
                for i = 1, 4 do
                    bias[i] = bias[i] * 0.05 -- weaken everything
                end
